...
#   nix develop -c b0/run_components.sh
set -euo pipefail
R=$PWD; S=b0/out/stim; O=b0/out/comp; mkdir -p $O
  ...
  for m in hdrvdp:V2_hdrvdpmtf cie:V3_cie99; do
    if [ $c = C_src1 ]; then
      ...
      for q in ped_src ped; do
        python3 tracks/hdrvdp3/img2raw.py $O/$q.exr $O/$q.raw; oct $O/$q.raw $O/${m#*:}_$q ${m%%:*} 2 | tail -1
      ...
      oct $O/$c.raw $O/${m#*:}_$c ${m%%:*} 2 | tail -1
