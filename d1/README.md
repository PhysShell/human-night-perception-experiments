...
- [x] D1-B4 independent chroma-collapse stage (`d1/chroma_b4/`, pre-registered): Wanat–Mantiuk s(L) passes 7/8 gates (fails the monotone-collapse gate on the yellow patch only: hue rotation through neutral); pcond `-c` law erases the mesopic tint and most lamp colour; S1 = neutral grey night + warm lamps. s(L) constant still to be verified against the paper
