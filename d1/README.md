...
- [x] **Axis A FROZEN**: pcond-derived scotopic luminance mapping (`-s -c`, luminance only; since D1-AX read *before* pcond's `clipgamut`). PASS = a deterministic luminance donor satisfying D1's necessary conditions on our scenes; not a claim that Ward Larson/Ferwerda reproduce night brightness appearance exactly
...
- [x] D1-AX axis-A extraction (`d1/a_extract/`, pre-registered; run 1 FAIL on a DO_LINEAR unit misreading, v2 pre-registered correction): **PASS, FROZEN**. Y_A = Rec.709 luminance of pcond's *pre-gamut* output, reproduced exactly (verbatim `clipgamut(greypoint)` of the prediction matches pcond's RGB on 100 % of pixels of every image). F1 now monotone for all colours (blue 17.9 → 24.5 → 49.9 → 91.2). Finding: on all real stills pcond `-s` falls back to its linear operator (Ward 1994 scale factor; `mkbrmap`: no compression needed); the histogram map is active only on F1
- [x] D1-DR display realisation (`d1/display_r/`, pre-registered; v1 both KILLED, one by an sRGB encoder/decoder knee artefact of our own measurement; v2 pre-registered correction): **Y-priority FROZEN** (keep Y_A, minimum chroma reduction towards white at fixed Y; all gates pass). C-priority KILLED: F1 red/blue non-monotone (Y_max falls as B restores saturation) + non-minimal on the mixed class. Price: S1 lamp cores (requested ≥ 90 cd/m²) go white; shoulders keep full colour and Y. S5 blue sky stays = B residual
- [x] D1-P4v2 + R3 (`d1/p4v2/`, pre-registered): **PASS everywhere, all predictions held**. 0 inversions on ~2·10⁸ resolvable pairs per image vs pcond's own L_eff (pairs closer than their `matscan` von Kries factor excluded, counted as diagnostic: F1 0.31 %, S4/S5 0.023 %); functional self-check Y_out = clamp(F(L_eff)·k) within 1e-7 everywhere; S3_nobar N/A + R3-flat PASS. Old P-4 failures (S4 0.937, S5 0.968) = intended `-c` reorder
- [x] **D1 final acceptance (`d1/final/`, pre-registered): ACCEPTED, D1 CLOSED.** P-1…P-9 all PASS on S0, S1, S2 (48 frames), S3_bar/nobar, S4, S5, F1 with frozen A (pcond `-s -c`, pre-gamut) + B (Cao/Kirk κ = 3 + Wanat-derived chroma collapse) + Y-priority display. Documented, non-failing residuals: S1 lamp cores white at the 100 cd/m² peak; S5 blue sky (B)
- [ ] (optional, outside D1) cross-donor write-up; axis C behind its C0 kill-gate
