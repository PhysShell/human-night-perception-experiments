...
S=$SECONDS
# resume: HAZE_START / LAMPS_START / OCC_START skip frames already on disk
env "${common[@]}" M25_START="${HAZE_START:-1}" M25_PASS=haze blender -b --factory-startup --python m1/scene.py -- \
  "$OUT/haze_####" "$HAZE_SPP" "$ATM" > "$OUT/haze.log" 2>&1
echo "haze: $FRAMES frames, $((SECONDS - S)) s"; S=$SECONDS
env "${common[@]}" M25_START="${LAMPS_START:-1}" M25_PASS=lamps M25_LAMP_PX=0.7 M25_SS=4 blender -b --factory-startup --python m1/scene.py -- \
...
env "${common[@]}" M25_START="${OCC_START:-1}" M25_PASS=occluders blender -b --factory-startup --python m1/scene.py -- \
